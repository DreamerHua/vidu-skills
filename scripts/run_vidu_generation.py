...
Options:
  ...
  --image path             (img2video: 1; headtailimg2video: 2; character2video: 2-7)
...
def main() -> int:
    ...
    if args.task_type == "headtailimg2video" and len(images) != 2:
        ...
        return 1
    if args.task_type == "character2video" and (len(images) < 2 or len(images) > 7):
        print(json.dumps({"error": "character2video requires 2-7 --image"}), file=sys.stderr)
        return 1

    image_uris = []
    if args.task_type in ("img2video", "headtailimg2video", "character2video") and images:
        for path in images:
