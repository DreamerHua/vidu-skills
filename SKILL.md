...
- **参考生视频 (character2video)**: **2–7 张图 + 一段文字**。仅 Q2，时长 2–8，**不传 transition**。主体（material element）**仅在此类任务中使用**；在 prompts 中通过 `type: "material"`、`material.id`、`material.version` 引用。
- **创建主体 (Create element)**: 上传 1–3 张图片，提供主体名称和描述，调用 **POST** `/vidu/v1/material/elements`；图片需先经现有上传流程得到 `ssupload:?id=...`。响应含主体 `id` 与 `version`，用于参考生任务。
...
- **参考生视频**: **2–7 张图 + 1 段文字**，`--type character2video`；仅 Q2，**不要传 transition**。

脚本会根据 `--type` 自动省略不该传的参数（如 img2video 不传 aspect_ratio，character2video 不传 transition）。选 type 和 model-version 时请严格按上表。

---

## 创建主体（POST /vidu/v1/material/elements）

创建主体用于在参考生任务中使用的 material element：上传 1–3 张图、名称与描述，接口返回主体 id 与 version。

- **URL**: `https://service.vidu.cn/vidu/v1/material/elements`（或 `{VIDU_BASE_URL}/vidu/v1/material/elements`）
- **Method**: POST
...
**Request body**: `name`（主体名称）, `modality: "image"`, `type: "user"`, `components`（1–3 个：第一个 `type: "main"`，其余 `type: "auxiliary"`；每项含 `content`、`src_img` 均为 `ssupload:?id=...`，`content_type: "image"`）, `version: "0"`, `recaption: { description, style? }`。详见 **references/api_reference.md** §3c。
...
主体**仅可在参考生任务（character2video）**中使用。提交 **POST** `/vidu/v1/tasks` 时：`type: "character2video"`；`input.prompts` 需包含 (1) 文本 prompt，如 `content: "[@主体名]"`，(2) material prompt：`type: "material"`, `name`（主体名）, `material: { "id": "<element_id>", "version": "<element_version>" }`。`id` 与 `version` 来自创建主体或查询主体接口的返回。
...
5. **参考生视频 (character2video)**  
   **2–7 张图 + 1 段文字**，仅 Q2。`run_vidu_generation.py character2video --prompt "<user text>" --image <path1> --image <path2> [...]`（不要传 transition）。
...
- **Submit**: `python scripts/submit_task.py --type text2video|img2video|headtailimg2video|character2video --prompt "..." [--image-uri "ssupload:?id=..."] [...]`; capture task_id from stdout. Respect 任务支持列表 (img2video 不传 aspect_ratio，character2video 不传 transition，图片数量 headtail=2、character2video=2–7、img2video=1).
